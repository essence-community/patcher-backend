...
fun createClassPatch(handle: Handle, config: Config, bdQuery: MDB): Boolean {
    ...
    handle
        ...
        .forEach { cl ->
            ...
            listAll.addAll(clAttr)
            listAll.add(MCleanObjectAttr(cl.ckId, attrs))
        }
    ...
    listAll.addAll(
        handle
            ...
            .define(
                "FILTER",
                "and ck_class_parent not in (select mc.ck_class from $nameTableModuleClass mc) and ck_class_child not in (select mc.ck_class from $nameTableModuleClass mc)"
            )
            .map({ rs, _ ->
                MClassHierarchy(
                    rs.getString(1),
    ...
    listAll.add(MAttrClean())
    if (config.type == DBType.POSTGRES) {
        writeChangePostgres(config.patchDir, "MetaClass", listAll)
    }
    return true
}
